...
export HBASE_MASTER_OPTS="-Xms{{ pillar.cdh4.hbase.master_initial_heap }}m -Xmx{{ pillar.cdh4.hbase.master_max_heap }}m -Xmn{{ pillar.cdh4.hbase.master_young_gen }} -XX:+UseConcMarkSweepGC -XX:+AggressiveOpts -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -Xloggc:{{ pillar.cdh4.hbase.log_dir }}/hbase-master-gc.log -Djute.maxbuffer={{ pillar.cdh4.hbase.jute_maxbuffer }}" 

export HBASE_REGIONSERVER_OPTS="-Xms{{ pillar.cdh4.hbase.region_initial_heap }}m -Xmx{{ pillar.cdh4.hbase.region_max_heap }}m -Xmn{{ pillar.cdh4.hbase.region_young_gen}} -XX:+UseConcMarkSweepGC -XX:+CMSIncrementalMode -XX:ParallelGCThreads=8 -XX:+AggressiveOpts -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -Xloggc:{{ pillar.cdh4.hbase.log_dir }}/hbase-regionserver-gc.log"

HBASE_LOG_DIR={{ pillar.cdh4.hbase.log_dir }}

{% if not pillar.cdh4.hbase.manage_zk %}
